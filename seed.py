...
_PAST_INCIDENTS = [
    ...
    {
        "title": "auth-api brute-force credential stuffing attack detected",
        "root_cause": "A sudden spike of 15,000 failed authentication requests from 42 unique IP addresses targeting endpoint /login. High volume saturated auth-api thread pool.",
        "action_taken": "Temporarily blocked the offending IP ranges via firewall rules; enabled recaptcha gate on login endpoint.",
        "resolution_notes": "No customer accounts were compromised. Decreased rate-limit threshold for failed login attempts to 5 per IP per minute.",
        "service": "auth-api",
        "occurred_at": datetime.now(timezone.utc) - timedelta(days=2),
    },
    {
        "title": "payments-api anomalous outbound traffic spike (data exfiltration attempt)",
        "root_cause": "A compromised debug endpoint /v1/debug/dump was queried repeatedly, resulting in 2.5 GB of anomalous outbound HTTPS traffic to an unlisted external destination.",
        "action_taken": "Disabled the /v1/debug/dump endpoint; revoked and rotated payments-api DB credentials; blocked destination IP at the NAT gateway.",
        "resolution_notes": "Data exfiltration attempt mitigated. Added egress filtering rules to restrict payments-api to approved payment processor domains only.",
        "service": "payments-api",
        "occurred_at": datetime.now(timezone.utc) - timedelta(days=6),
    },
    {
        "title": "checkout-api repeated unauthorized access to internal administration endpoint",
        "root_cause": "Repeated HTTP GET and POST requests targeting /admin/config from various external IPs with invalid or expired admin tokens, attempting to bypass security checks.",
        "action_taken": "Configured network access control list (ACL) to restrict access to the /admin route exclusively to internal private networks.",
        "resolution_notes": "Endpoint access is now protected by mandatory VPN and OAuth2 bearer tokens. Updated alerting rule for unauthorized admin access attempts.",
        "service": "checkout-api",
        "occurred_at": datetime.now(timezone.utc) - timedelta(days=12),
    },
]
