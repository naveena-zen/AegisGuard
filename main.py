...
async def lifespan(app: FastAPI):
    logger.info("🚀 AegisGuard starting…")
    ...
    logger.info("🟢 AegisGuard is live on port 8000")
    yield

    scheduler.shutdown(wait=False)
    injector_task.cancel()
    logger.info("🔴 AegisGuard shut down")


# ── App ────────────────────────────────────────────────────────────────────────

app = FastAPI(
    title="AegisGuard — Autonomous Security Monitoring Agent",
    version="1.0.0",
    lifespan=lifespan,
)
