    ...
    <div className="login-wrap">
      <div className="login-card">
        <div className="login-logo">
          ...
          <h1>AegisGuard Security Console</h1>
          <p>Autonomous Threat Detection & Mitigation Agent</p>
        </div>
...
function SimilarIncidents({ items = [] }) {
  ...
  return (
    <div className="similar-incidents">
      <div className="step-label" style={{ marginBottom: 8 }}>📚 Similar past threats (RAG)</div>
      ...
      try {
        ...
        incs.forEach(inc => {
          if (!incidentIds.current.has(inc.id)) {
            ...
            addToast({
              title: `🚨 New security alert — ${inc.service}`,
    ...
    <div className="app">
      ...
      <header className="topbar">
        <div className="topbar-logo">
          ...
          AegisGuard
          <span style={{ fontWeight: 400, fontSize: 12, color: 'var(--text-muted)' }}>Autonomous Security Agent</span>
        </div>
        ...
        <div className="incidents-section">
          {incidents.length === 0 ? (
            <div className="empty-state">
              ...
              <div>No security alerts yet. Inject an anomaly to see the agent in action.</div>
            </div>
          ) : (
            incidents.map(inc => (
              <IncidentCard
