# AegisGuard Security Console Frontend

This is the interactive frontend dashboard for **AegisGuard**, the Autonomous Security Monitoring & Threat Investigation Agent. It is built using React and Chart.js.

## Features
- **Real-time Security Telemetry Dashboard**: Visualizes service metrics (CPU, memory, latency, error rates, and request rates) using Chart.js.
- **Threat Alert Timeline**: Renders the AI diagnostics loop trace, including tool calls, RAG search results, hypothesis generation, confidence metrics, and policies evaluated.
...
The Security Console is containerized using Nginx and can be run as part of the Compose stack:
```bash
...
## Demo Authentication
Access the dashboard using the default demo credentials:
- **Username**: `admin`
- **Password**: `vigil2025`
